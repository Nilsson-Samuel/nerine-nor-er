...
Takes raw pair lists from FAISS, phonetic, and MinHash blockers, enforces
same-type pairing, canonical ID ordering (a < b), deduplication, and produces
final candidate records with blocking_methods / blocking_source metadata.
"""
...
def union_candidates(
    ...
    def _add_pairs(
        raw: list[tuple[str, str]], method: str,
    ) -> int:
        added = 0
        for a, b in raw:
            if a == b:
    ...
    logger.info(
        "Candidate union input: faiss=%d, phonetic=%d, minhash=%d",
        n_faiss, n_phonetic, n_minhash,
    )
